...
class ProfilTelPage extends StatefulWidget {
  ProfilTelPageState createState() => ProfilTelPageState();
}

class ProfilTelPageState extends State<ProfilTelPage> {
        ...
        child: Scaffold(
            ...
            body: SingleChildScrollView(
              ...
              Container(
                  ...
                  child: Text("N° de téléphone actuel :",
                      style: TextStyle(fontWeight: FontWeight.w600))),
              SizedBox(height: 5),
              Container(
                  alignment: Alignment.centerLeft,
                  margin: EdgeInsets.only(left: 10, right: 10),
                  child: Text(globals.tel)),
              SizedBox(height: 30),
              Container(
                  alignment: Alignment.centerLeft,
                  margin: EdgeInsets.only(left: 10, right: 10),
                  child: Text("Nouveau N° de téléphone :",
                      style: TextStyle(fontWeight: FontWeight.w600))),
              SizedBox(height: 5),
              Container(
                  height: 50,
                      ...
                      onPressed: () async {
                        if (tel == "") {
                          utils?.showPopupWarning(
                              context, "Saisir un N° de téléphone");
                        } else {
                          await FirebaseFirestore.instance
                              .collection('users')
                              .doc(globals.user?.uid)
                              .update({'tel': tel}).then((value) async {
                            await utils
                                ?.openPopupSuccess(
